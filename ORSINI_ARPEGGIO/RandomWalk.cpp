...
/////SEQUNCIN NOTE
void RandomWalk::up(byte *seq, byte *scale, byte scaleSize){
	for(int i=0;i<16;i++){
		seq[i]=scale[(offset+(i%arp_loop_size))%(scaleSize)];
		//seq[i]=scale[random(16) ];
	}
}
void RandomWalk::down(byte *seq, byte *scale,  byte scaleSize){
	for(int i=0;i<16;i++){
				seq[i]=scale[((offset+(15-i))%arp_loop_size)%scaleSize];
	}
}

void RandomWalk::upDown(byte *seq, byte *scale,  byte scaleSize){
...
void RandomWalk::randomArp(byte *seq, byte *scale,  byte scaleSize){
	for(int i=0;i<16;i++){
		seq[i]=scale[(offset+random(15-offset)) % scaleSize];
	}
}

void  RandomWalk::makeSequence( byte *sequence, byte *scal, byte scalSize, byte direction, byte octave, byte arp_size, byte offst){
